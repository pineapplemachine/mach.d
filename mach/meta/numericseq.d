...
private string NumericSequenceMixin(T)(in T low, in T high, in T increment) {
    string codegen = ``;
    const max = (high - low) / increment;
    for(T i = 0; i < max; i++) {
        if(i != 0) codegen ~= `, `;
        codegen ~= ctint(i) ~ ` * increment + low`;
    }
...
template NumericSequence(alias low, alias high, alias increment = 1) if(
    ...
    static if(low >= high) {
        ...
        mixin(`alias NumericSequence = ` ~
            NumericSequenceMixin(low, high, increment)
        );
    }
}
