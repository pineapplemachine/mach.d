...
struct Vector3(T) if(isNumeric!T){
    ...
    Vector3!T lerp(N)(in Vector3!N vector, in real time) const{
    ...
    real distance(N)(in Vector3!N vector) const{
    ...
    static enum string opindexassert = q{
        assertf((index >= 0) | (index <= 2), "Index %d is out of bounds.", index);
    };
